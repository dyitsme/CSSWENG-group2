...
app.post('/uploadfile', async (req, res) => {
    ...
    if (Array.isArray(files)) {
        Account.findOne({ username: req.session.name }, async (err, user) => {
            files.forEach(file => {
                file.mv(path.resolve(__dirname, 'uploaded', file.name), (error) => {
                    if (directory == "") {
        ...
        Account.findOne({ username: req.session.name }, async (err, user) => {
            files.mv(path.resolve(__dirname, 'uploaded', files.name), (error) => {
                if (directory == "") {
    ...
    if (directory == "") {
        ...
        Account.findOne({ username: req.session.name }, (err, user) => {
            if (err){
                res.render('admanagerhome.hbs', { folders, files, link: "/admanagerhome", ID: "/register", Content:"Register a User", vError: 'visible' , oError: '1', type: 'error', mError: "Files failed to upload"});
            }

            if (user.role == "Administrator") {
                res.render('admanagerhome.hbs', { folders, files, link: "/admanagerhome", ID: "/register", Content:"Register a User", vSuccess: 'visible' , oSuccess: '1', type: 'success', mSuccess: "Your files are ready"});
            }
        })
    }
    else {
...
app.post('/change-selected-password', async (req, res) => {
    ...
    Account.updateOne({ username: req.query.text_search }, { pass: hashedPassword }, (error, success) => {
        if (success) {
            res.redirect('/search-user?text_search=' + req.query.text_search + "&success=password")
        }
    })
});
...
app.post('/edit-role', (req, res) => {
    Account.updateOne({ username: req.query.text_search }, { role: req.body.role }, (error, success) => {
        if (success) {
            res.redirect('/search-user?text_search=' + req.query.text_search + "&success=role")
        }
    })
});
...
app.get('/delete-user', (req, res) => {
    Account.deleteOne({ username: req.query.text_search }, (error, success) => {
        if (success) {
            Account.findOne({ username: req.session.name }, (err, user) => {
                if (user.role == "Administrator") {
                    return res.render('profile.hbs', { link: "/admanagerhome", profilename: req.session.name, role: user.role, C: "regisbutton", act: "redirectRegister()", content: "Register an Account", vSuccess: 'visible', oSuccess: '1', type: 'success', mSuccess: "User deleted" });
                }
                else if (user.role == "Manager") {
                    return res.render('profile.hbs', { link: "/admanagerhome", profilename: req.session.name, role: user.role, design: "trap", vSuccess: 'visible', oSuccess: '1', type: 'success', mSuccess: "User deleted" })
                }
            });
        }
    })
...
app.get('/filterSelected', async(req, res)=>{
        ...
        if(directory == ""){
            ...
            Account.findOne({username: req.session.name}, (err, user)=>{
                if(user.role == "Administrator"){
                    res.render('admanagerhome.hbs', {folders,files, link: "/admanagerhome", ID: "/register", Content:"Register a User",movFolder: folders1, movemodal:"visibility:visible", blockermodal:"display:block",path:directory});
                }
                else if(user.role == "Manager"){
                    
                    res.render('admanagerhome.hbs',{link: "/admanagerhome", design:"trap", folders, files, movFolder:folders1,movemodal:"visibility:visible", blockermodal:"display:block",path:directory })
                }
            })
        }
})
app.get('/moveAction', (req, res)=>{
    Account.findOne({username:req.session.name}, (err, user)=>{
        if(folIDmove == ""){
            console.log("START");
